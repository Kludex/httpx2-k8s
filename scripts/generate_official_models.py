...
OUTPUT_PATH = PROJECT_ROOT / "src" / "httpx2_k8s" / "_official_models.py"
PUBLIC_OUTPUT_PATH = PROJECT_ROOT / "src" / "httpx2_k8s" / "models.py"
PUBLIC_STUB_OUTPUT_PATH = PROJECT_ROOT / "src" / "httpx2_k8s" / "models.pyi"
...
def _render_public_module(schemas: Mapping[str, object]) -> str:
    names = tuple(sorted(_model_names(schemas).values()))
    return "\n".join(
        [
            "# Generated by scripts/generate_official_models.py; do not edit by hand.",
            "# ruff: noqa: F822",
...
def _render_public_stub(schemas: Mapping[str, object]) -> str:
    names = tuple(sorted(_model_names(schemas).values()))
    return "\n".join(
        [
            "# Generated by scripts/generate_official_models.py; do not edit by hand.",
            ...
            *(f"    {name} as {name}," for name in names),
            ")",
            "",
            "__all__: list[str]",
            "",
...
def main() -> None:
    ...
    OUTPUT_PATH.write_text(_render(schemas, schema_versions))
    PUBLIC_OUTPUT_PATH.write_text(_render_public_module(schemas))
    PUBLIC_STUB_OUTPUT_PATH.write_text(_render_public_stub(schemas))
    subprocess.run(
        [
            sys.executable,
            "-m",
            ...
            str(PUBLIC_STUB_OUTPUT_PATH),
        ],
        check=True,
    )
    print(f"Generated {len(schemas)} official models in {OUTPUT_PATH} and {PUBLIC_OUTPUT_PATH}")
