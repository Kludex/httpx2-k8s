...
def _model_import_lines(model_names: set[str], names: Mapping[str, str]) -> list[str]:
    canonical_by_name = {name: canonical for canonical, name in names.items()}
    by_module: defaultdict[str, list[str]] = defaultdict(list)
    for name in model_names:
        by_module[_model_module(canonical_by_name[name])].append(name)
    return [
        line
        for module, imported_names in sorted(by_module.items())
        for line in (
            f"from {module} import (",
            *(f"    {name}," for name in sorted(imported_names)),
            ")",
        )
    ]
...
def _render_facade_module(
    ...
    *,
    ...
    lines = [
        ...
        *_model_import_lines(model_imports, names),
        "",
        f"class {mixin}:",
        f"    _client: {protocol}",
        *initializer,
...
def _render_combined_facade(config: APIConfig) -> str:
    module_prefix = _module_prefix(config)
...
def _api_export_modules(config: APIConfig, *, combined: bool) -> dict[str, str]:
    module_prefix = _module_prefix(config)
    return {
        config["class_name"]: f"{module_prefix}.{'_facade' if combined else '_sync'}",
        f"Async{config['class_name']}": f"{module_prefix}.{'_facade' if combined else '_async'}",
    }


def _render_public_init(
    config: APIConfig,
    ...
    *,
    ...
    module_prefix = _module_prefix(config)
    imports = {
        **_api_export_modules(config, combined=combined),
        **{name: f"{module_prefix}._models" for name in model_names},
    }
    return "\n".join(
        [
            "# Generated by scripts/generate_official_apis.py; do not edit by hand.",
            ...
            "",
            f"_LAZY_IMPORTS = {imports!r}",
            "",
            "def __getattr__(name: str) -> object:",
            "    return load_export(name, __name__, globals(), _LAZY_IMPORTS)",
            "",
            f"__all__ = {sorted(imports)!r}",
            "",
        ]
    )


def _render_public_stub(
    config: APIConfig,
    model_names: Sequence[str],
    *,
    combined: bool,
    ...
    module_prefix = _module_prefix(config)
    api_module = "_facade" if combined else "_sync"
    async_module = "_facade" if combined else "_async"
    exports = sorted((config["class_name"], f"Async{config['class_name']}", *model_names))
    return "\n".join(
        [
            "# Generated by scripts/generate_official_apis.py; do not edit by hand.",
            "# ruff: noqa",
            ...
            "",
            (
                f"from {module_prefix}.{api_module} import "
                f"{config['class_name']} as {config['class_name']}"
            ),
            (
                f"from {module_prefix}.{async_module} import "
                f"Async{config['class_name']} as Async{config['class_name']}"
            ),
            *(
                [
                    f"from {module_prefix}._models import (",
                    *(f"    {name} as {name}," for name in model_names),
                    ")",
                ]
                if model_names
                else []
...
def _write_api(
    ...
    *,
    ...
    model_module = f"{_module_prefix(config)}._models"
    model_names = tuple(
        sorted(
            name for canonical, name in names.items() if _model_module(canonical) == model_module
        )
    )
    paths = (
        root / "_operations.py",
    ...
    combined = tag in EXISTING_TAGS
    init_path.write_text(_render_public_init(config, model_names, combined=combined))
    stub_path.write_text(_render_public_stub(config, model_names, combined=combined))
    if combined:
        facade_path = root / "_facade.py"
        facade_path.write_text(_render_combined_facade(config))
        return (*paths, facade_path, init_path, stub_path)
    public_paths = (root / "_sync.py", root / "_async.py")
    ...
    return (*paths, *public_paths, init_path, stub_path, group_init)
