...
def test_rbac_v1_lifecycles_through_httpx2() -> None:
    ...
    with KubeClient(
        ...
        assert client.rbac_v1.read_namespaced_role("config reader", "team one") == role
        ...
        role = client.rbac_v1.replace_namespaced_role(
            "config reader",
            "team one",
            role,
            field_manager="rbac-tests",
        ...
        assert client.rbac_v1.list_role_for_all_namespaces(
            label_selector="owner=tests",
            field_selector="metadata.name=config reader",
            limit=1,
        ...
        assert [
            item.metadata.name
            for item in client.rbac_v1.delete_collection_namespaced_role(
                "team one",
        ...
        assert (
            client.rbac_v1.read_namespaced_role_binding("read configs", "team one") == role_binding
        )
        ...
        role_binding = client.rbac_v1.replace_namespaced_role_binding(
            "read configs",
            "team one",
            role_binding,
            field_manager="rbac-tests",
        ...
        assert client.rbac_v1.list_namespaced_role_binding("team one").items == [role_binding]
        assert client.rbac_v1.list_role_binding_for_all_namespaces(
            label_selector="owner=tests",
        ...
        assert [
            item.metadata.name
            for item in client.rbac_v1.delete_collection_namespaced_role_binding(
                "team one",
        ...
        assert (
            client.rbac_v1.delete_namespaced_role_binding("read configs", "team one").status
            == "Success"
        )
        assert (
            client.rbac_v1.delete_namespaced_role("config reader", "team one").status == "Success"
        )
        ...
        assert client.rbac_v1.read_cluster_role("health reader") == cluster_role
        ...
        cluster_role = client.rbac_v1.replace_cluster_role(
            "health reader",
            cluster_role,
            field_manager="rbac-tests",
            dry_run="All",
        )
        assert client.rbac_v1.list_cluster_role(label_selector="owner=tests").items == [
            cluster_role
        ]
        assert [
            item.metadata.name
            for item in client.rbac_v1.delete_collection_cluster_role(
        ...
        assert client.rbac_v1.read_cluster_role_binding("health readers") == cluster_binding
        ...
        cluster_binding = client.rbac_v1.replace_cluster_role_binding(
            "health readers",
            cluster_binding,
            field_manager="rbac-tests",
            dry_run="All",
        )
        assert client.rbac_v1.list_cluster_role_binding().items == [cluster_binding]
        assert [
            item.metadata.name
            for item in client.rbac_v1.delete_collection_cluster_role_binding(
                DeleteOptions(dry_run=["All"], propagation_policy="Orphan"),
                label_selector="owner=tests",
            ).items
        ] == ["health readers"]
        assert client.rbac_v1.delete_cluster_role_binding("health readers").status == "Success"
        assert client.rbac_v1.delete_cluster_role("health reader").status == "Success"
