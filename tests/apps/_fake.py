...
class FakeAppsAPI:
    """Stateful Apps v1 API boundary for workload controllers."""
    ...
    def __call__(self, request: httpx2.Request) -> httpx2.Response:
        ...
        if request.method == "DELETE" and name is None:
            ...
            selected_items = [current for _, current in selected]
        ...
        if request.method == "PATCH":
            ...
            params = dict(request.url.params)
            self.patch_calls.append((resource, content_type, params))
            if content_type == "application/apply-patch+yaml":
                body = cast(dict[str, object], json.loads(request.content))
                metadata = cast(dict[str, object], body["metadata"])
                metadata.update(namespace=namespace, resourceVersion="2", uid=f"{resource}-uid")
                self._add_status(resource, body)
                ...
                annotations = cast(dict[str, str], patch_metadata["annotations"])
...
def template(app: str) -> PodTemplateSpec:
    return PodTemplateSpec(
        metadata=ObjectMeta(labels={"app": app}),
        spec=PodSpec(
            containers=[Container(name="workload", image="registry.invalid/workload:1")],
