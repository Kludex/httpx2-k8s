...
def test_all_storage_v1_lifecycles_through_httpx2() -> None:
    ...
    with KubeClient(
        ...
        assert client.storage_v1.read_storage_class("archive class") == storage_class
        ...
        storage_class = client.storage_v1.replace_storage_class(
            "archive class",
            storage_class,
        ...
        classes = client.storage_v1.list_storage_class(
        ...
        deleted_classes = client.storage_v1.delete_collection_storage_class(
        ...
        assert (
            client.storage_v1.delete_storage_class("archive class").metadata.name == "archive class"
        )

        driver = client.storage_v1.create_csi_driver(
            CSIDriver(
        ...
        assert client.storage_v1.read_csi_driver("storage.httpx2-k8s.invalid") == driver
        driver = client.storage_v1.apply_csi_driver(
        ...
        assert client.storage_v1.list_csi_driver().items == [driver]
        deleted_drivers = client.storage_v1.delete_collection_csi_driver(
        ...
        assert (
            client.storage_v1.delete_csi_driver("storage.httpx2-k8s.invalid").metadata.name
            == "storage.httpx2-k8s.invalid"
        )
        ...
        assert client.storage_v1.read_csi_node("node one") == csi_node
        ...
        csi_node = client.storage_v1.replace_csi_node(
            "node one", csi_node, field_manager="replace-tests", dry_run="All"
        )
        assert csi_node.metadata.resource_version == "4"
        assert client.storage_v1.list_csi_node().items == [csi_node]
        deleted_nodes = client.storage_v1.delete_collection_csi_node(
        ...
        assert client.storage_v1.delete_csi_node("node one").metadata.name == "node one"

        capacity = client.storage_v1.create_namespaced_csi_storage_capacity(
            "team one",
            CSIStorageCapacity(
        ...
        assert (
            client.storage_v1.read_namespaced_csi_storage_capacity("zone capacity", "team one")
            == capacity
        )
        capacity = client.storage_v1.apply_namespaced_csi_storage_capacity(
            "zone capacity",
            "team one",
            capacity,
            field_manager="storage-tests",
        ...
        capacity = client.storage_v1.patch_namespaced_csi_storage_capacity(
            "zone capacity",
            "team one",
        ...
        capacity = client.storage_v1.replace_namespaced_csi_storage_capacity(
            "zone capacity",
            "team one",
            capacity,
            field_manager="replace-tests",
        ...
        assert client.storage_v1.list_namespaced_csi_storage_capacity("team one").items == [
            capacity
        ]
        assert client.storage_v1.list_csi_storage_capacity_for_all_namespaces(
            label_selector="owner=tests",
        ...
        assert (
            client.storage_v1.delete_namespaced_csi_storage_capacity(
                "zone capacity", "team one"
            ).status
            == "Success"
        )

        attachment = client.storage_v1.create_volume_attachment(
            VolumeAttachment(
        ...
        assert client.storage_v1.read_volume_attachment("attachment one") == attachment
        attachment = client.storage_v1.apply_volume_attachment(
            "attachment one",
            attachment,
            field_manager="storage-tests",
            dry_run="All",
        )
        attachment = client.storage_v1.patch_volume_attachment(
            "attachment one",
            MergePatch(document={"metadata": {"annotations": {"patched": "attachment"}}}),
            field_manager="storage-tests",
        )
        assert attachment.metadata.annotations == {"patched": "attachment"}
        attachment = client.storage_v1.replace_volume_attachment(
            "attachment one",
            attachment,
            field_manager="replace-tests",
            dry_run="All",
        ...
        attachment_status = client.storage_v1.read_volume_attachment_status("attachment one")
        assert attachment_status.status is not None
        assert attachment_status.status.attached is False
        attachment_status = client.storage_v1.patch_volume_attachment_status(
            "attachment one",
        ...
        attachment = client.storage_v1.replace_volume_attachment_status(
            "attachment one",
            attachment_status,
        ...
        assert client.storage_v1.list_volume_attachment().items == [attachment]
        ...
        assert (
            client.storage_v1.delete_volume_attachment("attachment one").metadata.name
            == "attachment one"
        )
        ...
        assert attributes_class.driver_name == "storage.httpx2-k8s.invalid"
        assert (
            client.storage_v1.read_volume_attributes_class("premium attributes") == attributes_class
        )
        ...
        attributes_class = client.storage_v1.replace_volume_attributes_class(
            "premium attributes",
            attributes_class,
            field_manager="replace-tests",
            dry_run="All",
        )
        assert client.storage_v1.list_volume_attributes_class(
            label_selector="owner=tests",
            field_selector="metadata.name=premium attributes",
            limit=1,
            continue_token="attributes-next",
        ).items == [attributes_class]
        assert client.storage_v1.delete_collection_volume_attributes_class(
            DeleteOptions(dry_run=["All"]), label_selector="owner=tests"
        ).items == [attributes_class]
        assert (
            client.storage_v1.delete_volume_attributes_class("premium attributes").metadata.name
            == "premium attributes"
        )
